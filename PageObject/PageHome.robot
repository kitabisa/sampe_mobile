...
${onboardTitle}                 id=com.kitabisa.android.staging:id/textViewTitle
...
Verify Pilihan Kitabisa Section
    scroll down to element              ${pilihanLihatSemuaBtn}         ${homepagecontainer}
...
Skip Onboarding
    ${result}=      run keyword and return status       wait until element is visible   ${lanjutButtonOnboarding}       timeout=7
    run keyword if  '${result}'=='True'     run keywords    repeat keyword  2 times     click element   ${lanjutButtonOnboarding}
    ...     AND     wait until element is visible       ${startDonationOnboarding}
    ...     AND     click element                       ${startDonationOnboarding}
...
Verify Doa Section
    Scroll Down to Element                 ${totalLikeDoa}            ${homepagecontainer}
